...
- **TechStack — редизайн под архитектурную схему.** Секция `TechStack` (`src/components/sections/TechStack.tsx`) полностью переписана: вместо orbit-диаграммы из Блока C — пирамида из 6 карточек-архитектуры (Frontend/Mobile → Backend/API → Daten/Integrationen & Services → Infrastruktur) с иконками `lucide-react` (`Monitor`/`Smartphone`/`Box`/`Database`/`Plug`/`Server`), SVG-коннекторами и точками между уровнями, мобильным fallback-списком (`md:hidden`, вертикальная линия слева). Справа — нумерованный таймлайн 01–06 с описанием каждой категории (`categoryDescriptions.*`) и бейджами технологий из `data/technologies.ts` (данные и типы не менялись — карточки берут `items.slice(0, limit)` напрямую). `messages/{de,en,ru}.json`: неймспейс `TechStack` пересобран — `headline` заменён на `headlineLine1/2`, добавлены `architectureAriaLabel`, `architectureTagline` (декоративная англоязычная подпись "Ideas / Architecture / Technologies / Real Impact" во всех трёх локалях, по аналогии с уже существующими `flowStart`/`flowEnd`/`process`/`outroLeft`/`outroRight`), `categoryDescriptions.*`; неиспользуемый `core.*` убран. Фон — `public/brand/background_stack_blueprint.png` (`next/image`, `fill`, `object-cover`, `opacity-60`); картинка несёт собственный вшитый декоративный текст по углам ("Architecture connects ideas" / "Build Integrate Deploy" / "Scalable Reliable Solutions"), дублировавший наши подписи в тех же местах — конфликт снят тремя размытыми «заплатками» цвета фона (`bg-bg`, `blur-2xl`, `rounded-full`) поверх этих зон, координаты вымерены по самому PNG (`System.Drawing`, кроп+чтение), а не CSS/SVG-маской: обе пробованные маски (`mask-composite: intersect` на нескольких `radial-gradient`, затем SVG `<mask>` с `objectBoundingBox` через CSS `mask-image: url(#id)`) оказались ненадёжны в Chrome для composite/референса маски на HTML-элементе. Визуально проверено через dev-сервер + Chrome-скриншоты на `/de`. `format` / `lint` / `typecheck` / `build` — чисто.
...
- **Блок H2** — светлая схема для остальных секций (About, Projects, Experience, Education, Services, Contact, Footer) и статус-цветов `ProjectCard` (лишился `--color-accent-tech` в Блоке H1). TechStack из этого списка уже вышел — получил полный редизайн отдельным шагом (см. выше).
